...
if [[ $pick == "1" ]] || [[ $pick == 1 ]]; then
    ...
    if [ -e "$file_path" ]; then
        cat "$file_path" | httpx -o alive.txt
        cat alive.txt | gau | waybackurls | hakrawler -d 2  >> urls.txt
        cat urls.txt | kxss | grep -E 'CSRF|csrf|token|TOKEN|anti-csrf|Anti-CSRF|anticsrf|AntiCSRF'
    else
    ...
    if [ -e "$file_path2" ]; then
        subfinder -l "$file_path2" | httpx -o alive.txt
        cat alive.txt | gau | waybackurls | hakrawler -d 2  >> urls.txt
        cat urls.txt | kxss | grep -E 'CSRF|csrf|token|TOKEN|anti-csrf|Anti-CSRF|anticsrf|AntiCSRF'
    else
    ...
    cat "$subdomain" | httpx -o alive.txt
    cat alive.txt | gau | waybackurls | hakrawler -d 2  >> urls.txt
    cat urls.txt | kxss | grep -E 'CSRF|csrf|token|TOKEN|anti-csrf|Anti-CSRF|anticsrf|AntiCSRF'
elif [[ $pick == "4" ]] || [[ $pick == 4 ]]; then
    read -p "enter the main domain: " domain
    subfinder -d "$domain" | httpx -o alive.txt
    cat alive.txt | gau | waybackurls | hakrawler -d 2  >> urls.txt
    cat urls.txt | kxss | grep -E 'CSRF|csrf|token|TOKEN|anti-csrf|Anti-CSRF|anticsrf|AntiCSRF'
